"""
Pinecone-based Products Classifier Module

Hybrid classification using Pinecone vector search with LLM fallback.
...
class PineconeProductClassifier:
    """Hybrid product classifier using Pinecone vector search with LLM fallback."""
    ...
    def __init__(
        ...
        """
        Initialize the Pinecone product classifier.
        ...
        pinecone_namespace : str
            Namespace within the index (e.g., "hs_0901")
        model_name : str
            LLM model name for fallback classification
        similarity_threshold : float
            Minimum similarity score for vector match (0.0-1.0)
        embedding_model : str
            Pinecone integrated embedding model (llama-text-embed-v2, multilingual-e5-large)
        text_field : str
        ...
        logger.info(f"Initializing Pinecone client for index '{pinecone_index_name}'")
        self.pinecone_client = PineconeClient(
            index_name=pinecone_index_name,
            embedding_model=embedding_model,
            text_field="content"  # Field name for text content
        )
        ...
        self.checkpoint_file = checkpoint_file or "pinecone_classification_checkpoint.pkl"
    ...
    def classify_single_pinecone(self, goods_description: str) -> Tuple[str, float, str]:
        """
        Classify a single goods description using Pinecone.
        ...
        try:
            ...
            matches = self.pinecone_client.search_with_rerank(
                namespace=self.pinecone_namespace,
                query_text=goods_description,
                top_k=10,  # Get more candidates for reranking
                rerank_top_n=5,
                rank_fields=[self.pinecone_client.text_field]  # Only one field supported
            )
            
            # Check if we have a good match
            if matches and matches[0]['score'] >= self.similarity_threshold:
                # Access fields using dict-style as per AGENTS.md
...
If uncertain, use "Unclassified". Array length MUST match {len(goods_descriptions)} items."""
    ...
    def classify_batch_hybrid(self, goods_batch: List[str]) -> List[Tuple[str, float, str]]:
        """
        Classify a batch using hybrid approach: Pinecone first, LLM for misses.
        ...
        # Try Pinecone for all items
        for i, goods in enumerate(goods_batch):
            category, confidence, method = self.classify_single_pinecone(goods)
        ...
        if llm_needed:
            ...
            # Store new classifications in Pinecone for future use
    ...
    def store_new_classifications(self, goods_list: List[str], categories: List[str]) -> None:
        """
        Store newly classified goods in Pinecone for future retrieval.
        ...
        try:
            ...
            if records:
                self.pinecone_client.upsert_records(
                    namespace=self.pinecone_namespace,
                    records=records,
                    batch_size=100
                )
                logger.debug(f"Stored {len(records)} new classifications in Pinecone")
